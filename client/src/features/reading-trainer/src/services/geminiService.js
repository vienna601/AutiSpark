...
- Recent performance: ${
      readingData.recentAnswerCorrect
...
Script:
    ...
    try {
      ...
      return (
        ...
        this.getFallbackTavusScript(readingData)
      );
    } catch (error) {
...
- Recent answers: ${recentAnswers
      .slice(-3)
      .map((a) => (a.correct ? "correct" : "incorrect"))
      .join(", ")}
...
- Specific, actionable feedback for the Tavus avatar to deliver
  ...
  static async evaluateAnswer(answerData) {
    const { userAnswer, correctAnswer, questionContext, storyData } =
      answerData;
...
- Encourage continued engagement
  ...
  static manualParseEvaluation(text) {
    ...
    const performance_level = text.includes("excellent")
      ? "excellent"
      : text.includes("struggling")
      ? "struggling"
      : text.includes("needs_support")
      ? "needs_support"
      : "good";
  ...
  static getFallbackEvaluation(readingData) {
    const accuracy =
      readingData.correctAnswers /
        (readingData.correctAnswers + readingData.incorrectAnswers) || 0;
  ...
  static createTranscriptAnalysisPrompt(transcriptData) {
    const studentMessages = transcriptData.filter(
      (msg) => msg.speaker === "Student"
    );
...
${transcriptData
  .map(
    (entry) =>
      `[${entry.timestamp.toLocaleTimeString()}] ${entry.speaker}: ${
        entry.text
      }`
  )
  .join("\n")}
...
Respond in JSON format:
{
  ...
  static generateRealTimeInsight(transcriptEntry, conversationHistory) {
    ...
    if (
      transcriptEntry.text.includes("um") ||
      transcriptEntry.text.includes("...")
    ) {
      return insights.hesitation;
    }
    if (
      transcriptEntry.type === "question" &&
      transcriptEntry.engagement === "high"
    ) {
