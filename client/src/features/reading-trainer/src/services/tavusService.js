...
console.log("Environment Variables Check:", {
  ...
  apiKeyLength: TAVUS_API_KEY?.length,
...
export class TavusService {
  ...
  static async cleanupActiveConversations() {
    try {
      console.log("🧹 Checking for active Tavus conversations...");

      const response = await fetch("/api/tavus/conversations/active", {
        method: "GET",
        headers: this.getHeaders(),
      });

      if (!response.ok) {
        console.warn("⚠️ Could not fetch active conversations");
        return [];
      }
      ...
      if (Array.isArray(conversations) && conversations.length > 0) {
        console.log(
          `Found ${conversations.length} active conversations. Closing them...`
        );
        await Promise.all(
          conversations.map((conv) =>
            this.endConversation(conv.conversation_id)
          )
        );
        console.log("✅ All active Tavus conversations closed.");
      } else {
      ...
      console.error("Error cleaning up Tavus conversations:", error);
      return [];
    }
  }
...
Remember: Your goal is to make reading feel safe, fun, and achievable. Every child learns at their own pace, and your job is to meet them where they are with love and encouragement.`;
  ...
  static async sendMessage(conversationId, message, readingContext = null) {
    ...
    try {
      const response = await fetch(
        `/api/tavus/conversations/${conversationId}/messages`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  static createContextualPrefix(readingContext) {
    ...
    let prefix = `CONTEXT: We're on page ${
      currentPage + 1
    } of ${totalPages} in "${storyTitle}".`;
    ...
    if (recentAnswerCorrect !== undefined) {
      prefix += ` Their last answer was ${
        recentAnswerCorrect ? "CORRECT" : "INCORRECT"
      }.`;
    }
  ...
  static async endConversation(conversationId) {
    ...
    try {
      const response = await fetch(
        `${TAVUS_API_URL}/conversations/${conversationId}/end`,
        {
          method: "POST",
          headers: {
            "x-api-key": TAVUS_API_KEY,
  ...
  static async deliverEvaluationFeedback(
    conversationId,
    evaluation,
    readingContext
  ) {
    ...
    const feedbackMessage = this.createContextualFeedback(
      evaluation,
      readingContext
    );
  ...
  static createContextualFeedback(evaluation, readingContext) {
    const { performance_level, tavus_speaking_points, specificPraise } =
      evaluation;
    const { currentPage, storyTitle, recentAnswerCorrect } = readingContext;

    let contextualMessage = "";

    contextualMessage += `We're reading "${storyTitle}" together on page ${
      currentPage + 1
    }. `;
  ...
  static async handleQuestionInteraction(
    conversationId,
    questionData,
    evaluation
  ) {
    ...
    if (evaluation.is_correct) {
      responseMessage = `${
        evaluation.tavus_response
      } ${evaluation.praise_points.join(". ")}! `;
    } else {
      responseMessage = `${
        evaluation.tavus_response
      } ${this.generateGentleCorrection(
        userAnswer,
        correctAnswer,
        storyContext
      )}`;
    ...
    if (
      evaluation.follow_up_suggestions &&
      evaluation.follow_up_suggestions.length > 0
    ) {
      responseMessage += ` ${this.createFollowUpQuestion(
        evaluation.follow_up_suggestions[0],
        storyContext
      )}`;
    }
  ...
  static createFollowUpQuestion(suggestion, storyContext) {
    const followUpQuestions = {
      ...
      comprehension:
        "Can you tell me what's happening in this part of the story?",
      connection: "Does this remind you of anything from your own life?",
    };

    return (
      followUpQuestions[suggestion.toLowerCase()] ||
      "What's your favorite part of this story so far?"
    );
  }
...
Stay in character as Alex, their encouraging reading tutor.
    ...
    return await this.sendMessage(conversationId, interactivePrompt, {
      mode: "interactive",
    });
  }
}
