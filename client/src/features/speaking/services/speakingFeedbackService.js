export class SpeakingFeedbackService {
  ...
  async analyzeSpeechAttempt(question, spokenText, goal) {
    try {
      ...
      const analysisPrompt = this.createSpeechPrompt(question, spokenText, goal);
...
Return ONLY the JSON object. No markdown. No backticks. No explanatory text.`;
  ...
  getDefaultSpeechFeedback() {
    return {
      overallScore: 8,
      encouragement: "Great job practicing your speaking! Every time you try, you're getting better.",
      strengths: [
  ...
  async testConnection() {
    try {
      ...
      const response = await fetch(requestUrl, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{ text: "Respond with exactly this JSON: {\"test\": \"success\", \"message\": \"Speaking service working!\"}" }]
          }]
        })
      });
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('❌ Speaking service test failed:', response.status, errorText);
        return false;
      }
    } catch (error) {
